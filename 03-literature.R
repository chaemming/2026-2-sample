...
flight_df <-data.frame(flights)#data frame으로 변환 !!매우 중요!!
str(flight_df)


##month=2인 자료만 필터링, 원본데이터는 그대로임.
flight_df %>% 
  filter(month==2)
##month=2인 자료만 필터링해서 저장
flight_df %>% 
  filter(month==2) ->flight_df_filter_month1
...
##month=2가 아닌 자료만 필터링
flight_df %>% 
  filter(month!=2)  #느낌표는 not의 의미,입력은 ! = 으로 함. (띄어쓰기 없음)
##month가 5이상인 자료만 필터링
flight_df %>% 
  filter(month >=5) 
